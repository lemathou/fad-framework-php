...
// Templates
$template_list = array();
$query = db()->query(" SELECT t1.id , t1.name , t2.title FROM _template as t1 LEFT JOIN _template_lang as t2 ON t1.id=t2.id AND t2.lang_id=".SITE_LANG_ID);
while ($template = $query->fetch_assoc())
{
	if (!$template["title"])
		$template["title"] = $template["name"];
	$template_list[$template["id"]]["id"] = $template["id"];
	if (count($e=explode("/",$template["name"]))>1)
	{
		$template["type"] = $e[0];
		$template["title"] .= " <i>[$template[type]]</i>";
	}
	else
		$template["type"] = "";
	$template_list[$template["id"]]["type"] = $template["type"];
	$template_list[$template["id"]]["name"] = $template["name"];
	$template_list[$template["id"]]["title"] = $template["title"];
...
$query_string = " INSERT INTO `_page` ( `name` , `template_id` ) VALUES ( '".$page["name"]."' , '".$page["template_id"]."' ) ";
$query = db()->query($query_string);

if ($page["id"]=$query->last_id())
{
	$query_string = " INSERT INTO `_page_lang` ( `id` , `lang_id` , `url` , `titre` , `titre_court` ) VALUES ( '".$page["id"]."' , '".SITE_LANG_ID."' , '".$page["url"]."' , '".$page["titre"]."' , '".$page["titre_court"]."' ) ";
	$query = db()->query($query_string);
...
db()->query(" UPDATE `_page` SET `name` = '".addslashes($page["name"])."' , `template_id`='$page[template_id]' WHERE `id`='$page[id]' ");

db()->query(" UPDATE `_page_lang` SET `url` = '".addslashes($page["url"])."' , `titre` = '".addslashes($page["titre"])."' , `titre_court` = '".addslashes($page["titre_court"])."' WHERE `id`='$page[id]' AND `lang_id`='".SITE_LANG_ID."'");
...
if (isset($page["param"]) && is_array($page["param"]) && (count($page["param"]) > 0))
{
	...
	{
		$query_param_list[] = "( '$page[id]' , '$name' , '".addslashes($value)."' )";
	}
...
<tr>
	...
	<td><select name="update[template_id]"><option value="0">-- Sélectionner --</option><?php
	foreach ($template_list as $name=>$template)
	{
...
<tr>
	...
	<td><select name="insert[template_id]"><option value="0">-- Sélectionner --</option><?php
	foreach ($template_list as $name=>$template)
	{
		if ($template["id"] == $page["template_id"])
			echo "<option value=\"$template[id]\" selected>$template[title]</option>";
