...
{
	if (login_form['login[username]'].value && login_form['login[password]'].value)
	{
		login_form['login[password_crypt]'].value = calcMD5(LireCookie('PHPSESSID') + login_form['login[password]'].value);
		login_form['login[password]'].value = "";
		login_form.submit();
	}
	else
		return false;
